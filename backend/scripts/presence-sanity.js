...
const PRESENCE_USER = require("crypto").randomUUID();

if (!PEER_PUB || !PEER_PRIV) {
  console.error("Missing dummy peer keys. Run Step 6.5 (B) to generate/exports.");
  process.exit(2);
}
...
async function makeSigned(type, payload) {
...
(async () => {
  ...
  ws.on("open", async () => {
    // Teach the server our pubkey
    send(ws, { type: "SERVER_HELLO_LINK", from: PEER_ID, to: "*", ts: now(),
               payload: { url: "ws://dummy", pubkey_b64url: PEER_PUB } });

    // Signed advertise
    const adv = await makeSigned("USER_ADVERTISE", { user_id: PRESENCE_USER, server_id: PEER_ID, meta: { display_name: "Test User" } });
    setTimeout(() => send(ws, adv), 150);

    // Signed remove
    const rem = await makeSigned("USER_REMOVE", { user_id: PRESENCE_USER, server_id: PEER_ID });
    setTimeout(() => send(ws, rem), 600);
