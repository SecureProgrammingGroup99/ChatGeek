...
const TARGET    = require("crypto").randomUUID();
...
async function signed(type, payload) {
  const sig = await signPayload(payload, PEER_PRIV);
  return { type, from: PEER_ID, to: "*", ts: now(), payload, sig };
}

(async () => {
  const ws = new WebSocket(URL);
  ws.on("open", async () => {
    // Teach server our pubkey
    send(ws, { type:"SERVER_HELLO_LINK", from:PEER_ID, to:"*", ts:now(),
               payload:{ url:"ws://dummy", pubkey_b64url:PEER_PUB } });

    const start = await signed("FILE_START", { to_user: TARGET, file_id:"file-123", name:"demo.bin", size:3, sha256:"<sha256>" });
    setTimeout(()=> send(ws, start), 150);

    const c0 = await signed("FILE_CHUNK", { to_user: TARGET, file_id:"file-123", index:0, ciphertext:"Z29vZA" });
    const c1 = await signed("FILE_CHUNK", { to_user: TARGET, file_id:"file-123", index:1, ciphertext:"Z29vZA" });
    const c2 = await signed("FILE_CHUNK", { to_user: TARGET, file_id:"file-123", index:2, ciphertext:"Z29vZA" });
    setTimeout(()=> send(ws, c0), 280);
    setTimeout(()=> send(ws, c1), 360);
    setTimeout(()=> send(ws, c2), 440);

    const end = await signed("FILE_END", { to_user: TARGET, file_id:"file-123" });
    setTimeout(()=> send(ws, end), 520);
