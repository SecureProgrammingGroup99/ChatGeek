...
(async () => {
  ...
  ws.on("open", async () => {
    // teach server our pubkey for verification
    send(ws, { type: "SERVER_HELLO_LINK", from: PEER_ID, to: "*", ts: now(),
               payload: { url: "ws://dummy", pubkey_b64url: PEER_PUB } });

    // signed SERVER_DELIVER to a "local" user
    const payload = {
      user_id: PRESENCE_USER,     // set this to any id you want to simulate
      ciphertext: "Z29vZA",
      sender: require("crypto").randomUUID(),
    };
    const sig = await signPayload(payload, PEER_PRIV);
    setTimeout(() => send(ws, { type: "SERVER_DELIVER", from: PEER_ID, to: "*", ts: now(), payload, sig }), 200);
